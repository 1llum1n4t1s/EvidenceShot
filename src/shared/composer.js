// EvidenceShot Composer
// セッションベースの Canvas 合成・PNG iTXt メタデータ埋込・クリップボード PNG 生成を担う。
// 動作には DOM (document / canvas / document.fonts / createImageBitmap) が必要。
//
//   - Chrome (MV3): offscreen.html の中で読み込まれ、offscreen.js が薄いラッパー越しに呼ぶ
//   - Firefox (MV3): background.html (event page) の中で読み込まれ、background.js が直接呼ぶ
//
// API:
//   globalThis.EvidenceShotComposer.begin(sessionId, sessionSecret, meta)        -> {ok, error?}
//   globalThis.EvidenceShotComposer.addSlice(sessionId, sessionSecret, capture)  -> {ok, error?}
//   globalThis.EvidenceShotComposer.finalize(sessionId, sessionSecret)           -> {ok, fileName, downloadUrl, savedAsFormat, clipboardStatus, clipboardError, clipboardObjectUrl, error?}
//   globalThis.EvidenceShotComposer.abort(sessionId, sessionSecret)              -> {ok, error?}
//   globalThis.EvidenceShotComposer.revokeDownloadUrl(downloadUrl)               -> void
//   globalThis.EvidenceShotComposer.purgeAllSessions()                           -> void
(function initializeEvidenceShotComposer() {
  ...
  async function finalizeCaptureSession(sessionId, sessionSecret) {
    try {
      ...
      //  navigator.clipboard.write は「Document is not focused.」で失敗する)。
      // ここでは「PNG blob を準備して URL を作る」だけで、書込みは popup / content script に委譲。
      ...
      // offscreen / event page は chrome-extension:// オリジンで SW / background と
      // 同一パーティションのため、background 側の chrome.downloads.download({url: blobUrl})
      // からも解決できる。
  ...
  async function embedEvidenceMetadataIntoPng(blob, meta) {
    try {
      ...
      // chrome.runtime.getManifest() は offscreen / event page でも提供形態が安定しないため、
      // background 側で解決した値を meta.extensionVersion 経由で受け取る。
      const now = new Date();
      const fields = [
  ...
  globalThis.EvidenceShotComposer = Object.freeze({
    begin: beginSession,
    addSlice: addCaptureSlice,
    finalize: finalizeCaptureSession,
