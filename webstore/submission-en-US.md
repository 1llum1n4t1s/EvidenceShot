# EvidenceShot Store Submission Pack (en-US)

## 1. Basic Metadata
- Extension Name: `EvidenceShot`
- Version: `1.0.1`
- Suggested Category: `Developer Tools` or `Productivity`
- Store Language: `English (United States)`

## 2. Listing Copy (US English)
- Short description (<=132 chars):
  `Capture viewport or stitched screenshots for web testing, with optional timestamp and fixed footer text overlays.`
- Detailed description:
  `webstore/store-listing.en-US.txt`

## 3. Screenshots / Promo Assets
- 1280x800:
  - `webstore/images/01-feature-overview-1280x800.png`
  - `webstore/images/02-how-to-use-1280x800.png`
  - `webstore/images/03-hero-promo-1280x800.png`
- Small promo tile 440x280:
  - `webstore/images/promo-small-440x280.png`
- Marquee promo tile 1400x560:
  - `webstore/images/promo-marquee-1400x560.png`

## 4. Privacy Policy
- English: `docs/privacy-policy.md`
- Japanese: `docs/privacy-policy-ja.md`
- Public URL for submission:
  - `TODO: Add hosted HTTPS URL`

## 5. Permission Justifications (for CWS form)
- `activeTab`: Capture the current active tab after explicit user action
- `storage`: Store local settings such as format, timestamp style/size, and capture mode
- `scripting`: Inject capture control script into the current tab during capture
- `offscreen`: Compose and annotate images in a hidden document
- `downloads`: Save generated screenshots through Chrome downloads

## 6. Data Use Draft Answers
- Personal data collection: `No`
- Data sharing with third parties: `No`
- Sale/advertising use: `No`
- Tracking/analytics SDK: `None`
- Processing location: `Local browser runtime only`

## 7. Publisher Support Info (fill before submit)
- Support website URL: `TODO`
- Support email: `TODO`
- Developer/contact details: `TODO`

## 8. Final Checklist
- [ ] Privacy policy URL is publicly reachable over HTTPS
- [ ] Listing copy matches current functionality (WEBP, capture mode, footer text)
- [ ] At least 5 screenshots uploaded
- [ ] Permission explanations match manifest
